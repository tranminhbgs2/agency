...
class UserController extends Controller
{
    ...
    public function __construct(UserRepo $cateRepo, CustomerRepo $cusRepo, AgentRepo $agentRepo, BankAccountRepo $bankAccountRepo)
    {
    ...
    /**
     * API thêm mới KH từ CMS
     * URL: {{url}}/api/v1/transaction/store-agency
     *
     * @param StoreRequest $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function storeAgency(StoreRequest $request)
    {
        $params['username'] = request('username', null);
        $params['password'] = request('password', null);
        $params['fullname'] = request('fullname', null);
        ...
        $params['birthday'] = request('birthday', null);
        $params['address'] = request('address', null);
        $params['account_type'] = Constants::ACCOUNT_TYPE_AGENCY;
        $params['status'] = request('status', Constants::USER_STATUS_ACTIVE);
        $params['gender'] = request('gender', "P");
        $params['display_name'] = request('display_name', $params['fullname']);
        $params['name'] = request('name_agent', null); // ngân hàng
        ...
        $account_name = strtoupper($params['account_name']);
        ...
        if (empty($account_name) || empty($account_number) || empty($bank_code)) {
            ...
            return response()->json([
                'code' => 400,
                'error' => 'Thông tin tài khoản không được để trống',
                'data' => null
            ]);
        }
